...
def get_results(sampler_type):
    ...
    if MODE == "falsify":
        ...
        for i in range(1, 11):
            ...
            s0W = np.genfromtxt(sampler_type + "/falsify_csvs_" + str(i) + "/subscenario0W_results.csv", delimiter=",", names=True)
            s0B = np.genfromtxt(sampler_type + "/falsify_csvs_" + str(i) + "/subscenario0B_results.csv", delimiter=",", names=True)
        ...
        s0W = np.genfromtxt(sampler_type + "/smc_csvs/subscenario0W_results.csv", delimiter=",", names=True)
        s0B = np.genfromtxt(sampler_type + "/smc_csvs/subscenario0B_results.csv", delimiter=",", names=True)
...
def get_post_conditions(sampler_type):
    ...
    s0W = np.genfromtxt(sampler_type + "/smc_csvs/subscenario0W_post_conditions.csv", delimiter=",", skip_header=True)
    s0B = np.genfromtxt(sampler_type + "/smc_csvs/subscenario0B_post_conditions.csv", delimiter=",", skip_header=True)
...
if MODE == "falsify":
    ...
    plt.savefig("plots/example2_falsify.pdf")
else:
    if SMC_GRAPH == "bar":
        ...
        def autolabel(rects, xpos='center', a=True):
            """
            Attach a text label above each bar in *rects*, displaying its height.
        ...
        # plt.show()
        plt.savefig("plots/example2_smc_bar.pdf")
    else:
        if SAMPLER_TYPE  == "random":
            ...
            plt.savefig("plots/example2_smc_line_random.pdf")
        else:
            ...
            plt.savefig("plots/example2_smc_line_halton.pdf")
